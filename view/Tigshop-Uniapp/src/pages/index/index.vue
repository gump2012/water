...
const getIndexData = async () => {
    ...
    try {
        ...
        let list = res.moduleList || [];
        
        // Ensure a searchBar exists at the top
        const hasSearch = list.some((item: any) => item.type === 'searchBar');
        if (!hasSearch) {
            list.unshift({
                type: 'searchBar',
                label: '搜索商品',
        ...
        // Filter to only match water delivery document requirements
        const allowedTypes = ['searchBar', 'notice', 'imageNav', 'imageAd', 'product'];
        let filteredList = list.filter((item: any) => allowedTypes.includes(item.type));
        
        // Sort modules exactly as described in the document
        const typeOrder = ['searchBar', 'notice', 'imageNav', 'imageAd', 'product'];
        filteredList.sort((a: any, b: any) => {
            return typeOrder.indexOf(a.type) - typeOrder.indexOf(b.type);
        });
        
        modulesData.value = filteredList;
