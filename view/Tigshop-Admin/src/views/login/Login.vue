...
const formSubmit = async (e: FormInstance | null) => {
    if (!e) {
        console.warn("[Login Debug] FormInstance is null");
        return;
    }
    console.log("[Login Debug] formSubmit started. Fetching CSRF...");
    await _getCsrf();
    console.log("[Login Debug] CSRF Token fetched:", csrfToken.value);
    
    try {
        await e.validate();
        console.log("[Login Debug] Form validation passed.");
    } catch (valError) {
        console.error("[Login Debug] Form validation failed:", valError);
        return;
    }
    ...
    try {
        ...
        console.log("[Login Debug] Sending adminSignin request. Data:", reqData, "CSRF:", csrfToken.value);
        const result = await (<SigninResult>adminSignin(
            reqData,
            csrfToken.value
        ));
        console.log("[Login Debug] adminSignin raw response success:", result);
        // 设置Token
        localStorage.setItem("accessToken", result.token ?? "");
        localStorage.setItem("adminType", result.adminType ?? "");
        console.log("[Login Debug] Token and AdminType stored in localStorage.");
        selectShopFlag.value = true;
        console.log("[Login Debug] selectShopFlag set to true.");
    } catch (error: any) {
        console.error("[Login Debug] adminSignin API request failed:", error);
        if (error.code == 1002) {
            console.log("[Login Debug] Catch error code 1002: showing slide verification slider");
            verify.value.show();
            ...
            console.error("[Login Debug] Network or other non-code error:", error);
            notification["error"]({
                message: "网络/系统错误",
                description: error.message || "请求失败，请检查后端服务是否正常运行且跨域允许"
            });
        }
    } finally {
        setTimeout(() => {
